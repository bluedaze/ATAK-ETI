...
def create_pipeline():
    ...
    face_nn.setBlobPath(str(Path(
        "models/face-detection-retail-0004/face-detection-retail-0004_openvino_2020.1_4shave.blob").resolve().absolute()))
    ...
    face_nn_xout.setStreamName("vehicle_nn")
    ...
    land_nn.setBlobPath(
        str(Path(
            "models/landmarks-regression-retail-0009/landmarks-regression-retail-0009_openvino_2020.1_4shave.blob").resolve().absolute())
    )
    ...
    pose_nn.setBlobPath(
        str(Path(
            "models/head-pose-estimation-adas-0001/head-pose-estimation-adas-0001_openvino_2020.1_4shave.blob").resolve().absolute())
    )
    ...
    gaze_nn.setBlobPath(
        str(Path(
            "models/gaze-estimation-adas-0002/gaze-estimation-adas-0002_openvino_2020.1_4shave.blob").resolve().absolute())
    )
...
class Main:
    ...
    def face_thread(self):
        face_nn = self.device.getOutputQueue("vehicle_nn")
    ...
    def get_frame(self, retries=0):
        if camera:
            return True, np.array(self.cam_out.get().getData()).reshape((3, 300, 300)).transpose(1, 2, 0).astype(
                np.uint8)
        else:
            ...
            if debug:  # face
                if self.gaze is not None and self.left_bbox is not None and self.right_bbox is not None:
                    ...
                    if args.lazer:
                        ...
                        for t in range(10)[::-2]:
                            cv2.line(beam_img, (re_x, re_y), ((re_x + x * 100), (re_y - y * 100)), (0, 0, 255 - t * 10),
                                     t * 2)
                            cv2.line(beam_img, (le_x, le_y), ((le_x + x * 100), (le_y - y * 100)), (0, 0, 255 - t * 10),
                                     t * 2)
                ...
                if not args.lazer:
                    ...
                    if self.nose is not None:
                        cv2.circle(self.debug_frame, (self.nose[0], self.nose[1]), 2, (0, 255, 0), thickness=5,
                                   lineType=8, shift=0)
                    if self.left_bbox is not None:
                        cv2.rectangle(self.debug_frame, (self.left_bbox[0], self.left_bbox[1]),
                                      (self.left_bbox[2], self.left_bbox[3]), (245, 10, 10), 2)
                    if self.right_bbox is not None:
                        cv2.rectangle(self.debug_frame, (self.right_bbox[0], self.right_bbox[1]),
                                      (self.right_bbox[2], self.right_bbox[3]), (245, 10, 10), 2)
